...
\begin{ppnote}
\textbf{\\ Relationship to other chapters:} This chapter provides the practical API for mutual exclusion using OpenMP locks. The conceptual treatment appears in Chapter~\ref{ch:mutual-exclusion}. The \texttt{single} directive introduced in Chapter~\ref{ch:thread-control} executes a region once; locks enable repeated, serialized execution by all threads.
\end{ppnote}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{singleVSlock.png}
